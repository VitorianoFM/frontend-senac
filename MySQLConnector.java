...
public class MySQLConnector { // Método que estabelece a conexão com o banco de dados MySQL.
    public static Connection conectar() throws Exception {
        ...
        String mysqlUrl = "jdbc:mysql://" + mysqlHost + ":" + mysqlPort + "/" + mysqlDb + "?user=" + mysqlUser + "&password=" + mysqlPassword; // String de conexão JDBC com o banco de dados MySQL.
        Connection conn = null; // Objeto de conexão que será retornado.
        try {
            Class.forName("com.mysql.cj.jdbc.Driver").getDeclaredConstructor().newInstance(); // Carrega a classe do driver JDBC do MySQL e cria uma variável.
            conn = DriverManager.getConnection(mysqlUrl); // Estabelece a conexão com o banco de dados.
            status = "Conexão realizada com sucesso!"; // Mensagem de sucesso na conexão.
        } catch (SQLException | InstantiationException | IllegalAccessException | ClassNotFoundException e) { // Captura qualquer exceção que ocorra durante a conexão e atualiza o status com a mensagem de erro.
